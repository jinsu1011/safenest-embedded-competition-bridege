(() => {
  ...
  async function reload() {
    const [remoteSpaces, remoteEvents] = await Promise.all([api("/api/spaces"), api("/api/events")]);
  ...
  function connectStream() {
    ...
    for (const type of ["sensor.updated", "space.created", "space.updated", "space.deleted"])
      eventStream.addEventListener(type, () => reload().catch(error => showToast(error.message)));
  }
