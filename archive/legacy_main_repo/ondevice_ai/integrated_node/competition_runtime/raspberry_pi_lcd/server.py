...
class SensorStore:
    ...
    def record_thermal_frame(self, payload: bytes, outer_sequence: int) -> None:
        ...
        plausible_pixels = sum(2332 <= value <= 4231 for value in pixels)
        if plausible_pixels < int(THERMAL_PIXEL_COUNT * 0.95):
            raise ValueError(
                "thermal frame contains too many implausible pixels: "
                f"{plausible_pixels}/{THERMAL_PIXEL_COUNT}"
            )
        ...
        with self._lock:
            ...
            self._thermal_min_c = round(min_raw * 0.1 - 273.15, 2)
            self._thermal_max_c = round(max_raw * 0.1 - 273.15, 2)
        ...
        with self._lock:
            ...
            return {
                **self._telemetry,
                ...
                "thermal": {
                    "available": self._thermal_payload is not None,
                    "fresh": thermal_fresh,
                    "width": THERMAL_WIDTH,
