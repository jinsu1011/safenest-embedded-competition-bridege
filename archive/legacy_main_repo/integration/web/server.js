...
async function pollRaspberryPi() {
  try {
    ...
    if (previousStatus !== item.status) {
      ...
      writeStore();
      await pushBridgeState(item.status);
    }
...
function holdOrMarkOffline(item, reason) {
  ...
  if (item.status !== "offline") {
    ...
    pushBridgeState("offline");
  }
}

async function pushBridgeState(status) {
  try {
    const response = await fetch(`${RPI_BRIDGE_URL}/api/state`, {
      method:"POST",
    ...
    console.error(`LCD 상태 전송 실패: ${error.message}`);
  }
}
