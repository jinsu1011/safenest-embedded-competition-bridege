...
MR60BHA2 ─┐
SCD40 ────┤
PIR ──────┼→ ESP32 ── TCP :9000 ─→ integration/pi_lcd/server.py
열화상 ───┘                         ├→ LCD Chromium /display
                                   ├→ HTTP :8080 /api/state
                                   └→ HTTP :8080 /api/thermal
                                               ↓ polling/proxy
                                     integration/web/server.js :3000
                                               ↓
                                      관리자/방문자 브라우저
```
...
`integration/pi_lcd/server.py`는 Python 표준 라이브러리 기반의 멀티스레드 서버입니다.
...
`integration/web/server.js`는 Express 기반이며 기본 포트는 3000입니다.
