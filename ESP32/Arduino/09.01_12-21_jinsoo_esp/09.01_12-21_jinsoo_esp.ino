...
constexpr uint32_t CO2_PREHEAT_MS = 180000;  // Winsen 규격 3분
...
bool co2InPreheat(uint32_t now) { return now < CO2_PREHEAT_MS; }
...
void acceptMhz19Sample(uint32_t now, uint16_t ppm) {
  ...
  if (co2InPreheat(now)) return;  // 예열 중 값은 모델 입력이 아닙니다
...
void publishTelemetrySnapshot(uint32_t now) {
  ...
  snapshot.co2Valid = !snapshot.co2Preheat && isFresh(lastCo2Ms, now, CO2_STALE_MS);
  if (snapshot.co2Preheat) {
      ...
      if (!sent) {
        ...
        if (!socketClosedByFailure && writeAttempted) {
          ...
          Serial.printf(
              ...
              "zero_writes=%u errno=%ld kept_open=1 timeouts=%lu\n",
              static_cast<unsigned long>(telemetry.sequence),
...
void logTransmittedTelemetry(uint32_t now) {
  ...
  Serial.printf(
      ...
      co2InPreheat(now) ? "preheat"
                        : (lastCo2Ms == 0 ? "never_seen"
                                          : (co2Fresh ? "ok" : "stale")),
...
void logAiReadiness(uint32_t now) {
  ...
  const bool preheatDone = !co2InPreheat(now);
...
void logHealth(uint32_t now) {
  ...
  Serial.printf(
      ...
      CO2_SENSOR_MODEL, CO2_EVENT_IDENTITY_CLASS, co2InPreheat(now) ? 1 : 0,
...
void runSelfTest() {
  ...
  if (got >= 9 && probe[0] == 0xFF && probe[1] == 0x86) {
    ...
    Serial.printf(
        ...
        static_cast<unsigned long>(CO2_PREHEAT_MS / 1000));
  } else {
    Serial.printf(
...
void setup() {
  ...
  Serial.printf("[wifi] connecting to %s (2.4 GHz 여야 합니다)\n", WIFI_SSID);
